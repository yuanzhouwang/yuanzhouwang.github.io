---
title: "Lab 9 - The t-test for Difference in Means"
...
df <- df %>%
  ...
  mutate(prepost = ifelse((Year == 2023 & Month >= 10) | Year > 2023, 1, 0))
```
...
```{r}
# [Your Code Here]
acled_dif <- mean(df$Fatalities[df$prepost == 1]) - mean(df$Fatalities[df$prepost == 0])
...
For a sample of $n_X$ independent and identically distributed (IID) random variables $X_1, \cdots, X_{n_X}$ and another sample of $n_Y$ IID random variables $Y_1, \cdots, Y_{n_Y}$, the standard error for the difference-in-means estimator when variances are unequal^[When variances are equal, it is better to use the pooled standard error, $\sqrt{\frac{s_{\text{total}}^2}{(\frac{1}{n_X} + \frac{1}{n_Y})^{-1}}}$] is given by:
...
```{r}
sd_post <- sd(df$Fatalities[df$prepost == 1])
sd_pre <- sd(df$Fatalities[df$prepost == 0])

n_post <- length(df$Fatalities[df$prepost == 1])
n_pre <- sum(df$prepost == 0)

acled_se <- sqrt(sd_post^2 / n_post + sd_pre^2 / n_pre)

s_total <- sd(df$Fatalities)

acled_se_varequal <- sqrt(s_total^2 * (1/n_post + 1/n_pre))
```
...
```{r}
ci_diff <- c(acled_dif - 1.96 * acled_se, acled_dif + 1.96 * acled_se)
ci_diff
```
...
You can conduct a t-test in R by using ``t.test(x_values, y_values, "two.sided", mu=0, var.equal = FALSE)``

```{r}
t.test(df[df$prepost == 1, ]$Fatalities, df[df$prepost == 0, ]$Fatalities, alternative = "two.sided", mu=0, var.equal = TRUE)
```

Let's apply what we've learned to answer the following questions:

- Is there a significant difference in the number of armed conflict events in the West Bank before vs. after October 2023?
- Is there a significant difference in the number of armed conflict events in the West Bank vs. Gaza Strip for the period before October 2023?
