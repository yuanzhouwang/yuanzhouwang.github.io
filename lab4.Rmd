...
#   mutate(gdp_total = gdpPercap * pop, rich = ifelse(lifeExp >= 20000, 1, 0))
...
- What should we expect for each? Do our diagnostics match what we should expect?
...
Let's try each of these in turn. Imagine we do not have the full gapminder dataframe, but only tables `A` and `B` above. How can we get the following?

- All countries in Africa, Europe, and Asia that have data for `lifeExp`, `gdpPercap`, and `gdp_total`.
- All countries in Africa, Europe, and Asia that have data for any one of these variables.
- All countries in Africa, Europe, and Asia that have data for `lifeExp` regardless of whether they have data for the other two
- All countries in Africa, Europe, and Asia that have data fro `gdpPercap` and `gdp_total` regardless of whether they have data for `lifeExp`.
